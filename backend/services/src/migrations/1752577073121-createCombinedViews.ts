...
CREATE OR REPLACE FUNCTION public.sum_json_recursively_fn(obj1 jsonb, obj2 jsonb)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    IF jsonb_typeof(obj1) = 'number' AND jsonb_typeof(obj2) = 'number' THEN
        RETURN to_jsonb((obj1::text)::numeric + (obj2::text)::numeric);
    END IF;

    IF jsonb_typeof(obj1) = 'array' AND jsonb_typeof(obj2) = 'array' THEN
        res := '[]'::jsonb;
        len := GREATEST(jsonb_array_length(obj1), jsonb_array_length(obj2));
        FOR i IN 0..len - 1 LOOP
            val1 := CASE WHEN i < jsonb_array_length(obj1) THEN obj1->i ELSE '0'::jsonb END;
            val2 := CASE WHEN i < jsonb_array_length(obj2) THEN obj2->i ELSE '0'::jsonb END;
            child := public.sum_json_recursively_fn(val1::jsonb, val2::jsonb);
            res := res || jsonb_build_array(child);
        END LOOP;
        RETURN res;
    END IF;
    ...
    IF jsonb_typeof(obj1) = 'object' AND jsonb_typeof(obj2) = 'object' THEN
        ...
        FOR key IN
            SELECT DISTINCT k
            FROM (
                SELECT jsonb_object_keys(obj1) AS k
                UNION
                SELECT jsonb_object_keys(obj2) AS k
            ) AS keys
        LOOP
            val1 := obj1->key;
            val2 := obj2->key;
            child := public.sum_json_recursively_fn(
                        COALESCE(val1, '0'::jsonb)::jsonb,
                        COALESCE(val2, '0'::jsonb)::jsonb
                     );
            res := jsonb_set(res, ARRAY[key], child, true);
        END LOOP;
        RETURN res;
    END IF;
